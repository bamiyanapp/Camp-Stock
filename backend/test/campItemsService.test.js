...
describe("campItemsService", () => {
  ...
  beforeEach(async () => {
    ...
    carCamp = { campId: "camp-1", name: "夏キャンプ", vehicleType: "car" };
  ...
  it("listForCamp: キャンプの移動手段に対応する持ち物のみを候補として返す", async () => {
    const result = await service.listForCamp("camp-1");
  ...
  it("listForCamp: 存在しないcampIdならNotFoundErrorを投げる", async () => {
    await expect(service.listForCamp("missing")).rejects.toThrow(/not found/);
  });

  it("setUsed(true): 持ち物を今回使うものとして選択する", async () => {
    const result = await service.setUsed("camp-1", "item-car", true);
    expect(result.used).toBe(true);
    expect(result.packed).toBe(false);

    const list = await service.listForCamp("camp-1");
  ...
  it("setUsed: 対応しない移動手段のitemIdでも例外を投げない（マスタに存在すれば選択可能）", async () => {
    const result = await service.setUsed("camp-1", "item-bike", true);
    expect(result.used).toBe(true);
  });

  it("setUsed: 存在しないitemIdならNotFoundErrorを投げる", async () => {
    await expect(service.setUsed("camp-1", "missing", true)).rejects.toThrow(
      /not found/
    );
  });

  it("setPacked: 使用中の持ち物の積み込み状態を更新する", async () => {
    await service.setUsed("camp-1", "item-car", true);
    const result = await service.setPacked("camp-1", "item-car", true);
  ...
  it("setPacked: 使用中でない持ち物に対してはNotFoundErrorを投げる", async () => {
    await expect(service.setPacked("camp-1", "item-car", true)).rejects.toThrow(
      /not marked as used/
    );
  });
  ...
  it("setUsed(false): 使用を解除すると積み込み状態もリセットされる", async () => {
    await service.setUsed("camp-1", "item-car", true);
    await service.setPacked("camp-1", "item-car", true);

    const result = await service.setUsed("camp-1", "item-car", false);
